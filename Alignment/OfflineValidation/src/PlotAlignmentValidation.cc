 ...
 */
void PlotAlignmentValidation::plotDMR(const std::string& variable, Int_t minHits, const std::string& options, const std::string& filterName) {
...
void PlotAlignmentValidation::plotDMRHistogram(PlotAlignmentValidation::DMRPlotInfo& plotinfo,
  ...
  if(plotinfo.filterName == "") { 
      ...
      for (uint i = 0; i<plotinfo.vars->getTree()->GetEntries(); i++) {
          reader.SetEntry(i);
          if ( *_entries < uint(plotinfo.minHits) ) continue;
          if ( *_subDetId !=  uint(plotinfo.subDetId) ) continue;
          if (direction != 0) {
              if (plotinfo.subDetId == 2) { // FPIX is split by zDirection 
                  if ( *_zDirection != direction ) continue;
              } else {
                  if ( *_rDirection != direction ) continue;
              }
          }
          if (layer > 0) {
              if ( *_layer != uint(layer) ) continue;
          }
          ...
          for(int ibad=0; ibad<tBadModules->GetEntries(); ibad++) {
              ...
              if(subdet=="BPIX"||subdet=="FPIX") {
                  if(*_bad_lumi <= 2.0) continue;   
              } else {
                  if(*_bad_lumi <= 7.0) continue; 
              }
              //modules that misbehave for less than 2/fb are OK = mild strict
              if(*_moduleId == uint(*_bad_id)) isBadModule = true;
          }

          if(isBadModule) continue;
