...
int main(int argc, char* argv[]) {
    ...
    std::map<Int_t,TString> Sublevel_Subdetector_Map = { {1,"PXB"}, {2,"PXF"}, {3,"TIB"}, {4,"TID"}, {5,"TOB"}, {6,"TEC"} };

    // wheels/layers per tracker phase: <pahse,<sublevel,number of layers/wheels>>
    const std::map<Int_t,std::map<Int_t,Int_t>> Phase_Subdetector_Layers_Map = { { 0 , { {1,3}, {2,2} } },
                                                                                 { 1 , { {1,4}, {2,3} } },
                                                                                 { 2 , { {1,4}, {2,10} } }
                                                                               };
        ...
        for (auto &sub_layer : Phase_Subdetector_Layers_Map.at(trackerphase)) {
            ...
            for (int iLayer = 1; iLayer <= sub_layer.second; iLayer++) {
                ...
                if ( sub_layer.first%2 != 0 ) {
                    ...
                    Sublevel_Subdetector_Map[100*sub_layer.first+iLayer] = Sublevel_Subdetector_Map[sub_layer.first]+"Layer"+TString(std::to_string(iLayer));

                }
                else {

                    Sublevel_Subdetector_Map[100*sub_layer.first+(1-1)*sub_layer.second+iLayer] = Sublevel_Subdetector_Map[sub_layer.first]+"Wheel"+TString(std::to_string(iLayer))+"Side1";
                    Sublevel_Subdetector_Map[100*sub_layer.first+(2-1)*sub_layer.second+iLayer] = Sublevel_Subdetector_Map[sub_layer.first]+"Wheel"+TString(std::to_string(iLayer))+"Side2";
    ...
    // variable mapping
    const std::map<Int_t,TString> Index_Variable_Map = { {0,"DX"}, {1,"DY"}, {2,"DZ"},
                                                         {3,"DAlpha"}, {4,"DBeta"}, {5,"DGamma"}
                                                       };
    const std::map<TString,TString> Variable_Name_Map = { {"DX","#Delta x"}, {"DY","#Delta y"}, {"DZ","#Delta z"},
                                                          {"DAlpha","#Delta #alpha"}, {"DBeta","#Delta #beta"}, {"DGamma","#Delta #gamma"}
                                                        };
    // estimator mapping
    ...
    for (auto &level : Sublevel_Subdetector_Map) {
        ...
        for (auto &variable : Index_Variable_Map) {
            ...
            Histos[level.first][variable.first] = new TH1D("Histo_"+level.second+"_"+variable.second,"Histo_"+level.second+"_"+variable.second, 1, -1, 1);
    ...
    for (auto &path_run_map : comparison_map) {
        ...
        while (Reader.Next()) {
            ...
            if (!usebadmodules) if (*badModuleQuality.Get()) continue;

            Int_t sublevel_idx = *sublevel.Get();

            Histos[sublevel_idx][0]->Fill(*dx.Get()*10000);
            Histos[sublevel_idx][1]->Fill(*dy.Get()*10000);
            Histos[sublevel_idx][2]->Fill(*dz.Get()*10000);
            Histos[sublevel_idx][3]->Fill(*dalpha.Get()*1000);
            Histos[sublevel_idx][4]->Fill(*dbeta.Get()*1000);
            Histos[sublevel_idx][5]->Fill(*dgamma.Get()*1000);

            if ( splitpixel && sublevel_idx <=2 ) {

                Int_t layer_index;

                if ( sublevel_idx%2 != 0 ) layer_index = 100*sublevel_idx+identifiers[2];
                else                       layer_index = 100*sublevel_idx+(identifiers[4]-1)*Phase_Subdetector_Layers_Map.at(trackerphase).at(sublevel_idx)+identifiers[3];

                Histos[layer_index][0]->Fill(*dx.Get()*10000);
                Histos[layer_index][1]->Fill(*dy.Get()*10000);
                Histos[layer_index][2]->Fill(*dz.Get()*10000);
                Histos[layer_index][3]->Fill(*dalpha.Get()*1000);
                Histos[layer_index][4]->Fill(*dbeta.Get()*1000);
                Histos[layer_index][5]->Fill(*dgamma.Get()*1000);
        ...
        for (auto &level : Sublevel_Subdetector_Map) {
            ...
            for (auto &variable : Index_Variable_Map) {
                    ...
                    if ( fabs(mean) > Graphs[level.first][variable.first][0]->GetMaximum() && fabs(mean) > 0. ) {
                        Graphs[level.first][variable.first][0]->SetMaximum( fabs(mean) );
                        Graphs[level.first][variable.first][0]->SetMinimum( -fabs(mean) );
                    }
    ...
    for (auto &level : Sublevel_Subdetector_Map) {
        ...
        for (auto &variable : Index_Variable_Map) {
            ...
            for (auto &estimator : Index_Estimator_Map) {
                ...
                Graphs[level.first][variable.first][estimator.first]->Write("Graph_"+level.second+"_"+variable.second+"_"+estimator.second);

                TString units = "mrad";
                if (variable.second.Contains("DX") || variable.second.Contains("DY") || variable.second.Contains("DZ")) units = "#mum";

                Trend trend( "Graph_"+level.second+"_"+variable.second+"_"+estimator.second,
                             "output",
                             "Trend",
                             Estimator_Name_Map.at(estimator.second)+"_{"+Variable_Name_Map.at(variable.second)+"} ["+units+"]",
                             -2 * fabs(Graphs[level.first][variable.first][estimator.first]->GetMinimum()),
                              2 * fabs(Graphs[level.first][variable.first][estimator.first]->GetMaximum()),
                             config_root,
                             lumi_per_run,
                             lumitype);
