...
#define COLOR_CODE(icolor) int(icolor / 4) + icolor + 1
  ...
  for (unsigned int ic = 0; ic <= NB_SUBLEVELS + 1; ic++) {
    c_global[ic] = new TCanvas(
        Form("global_%s_%d", ic == 0 ? "tracker" : (ic == 7 ? "pixel" : _sublevel_names[ic - 1].Data()), canvas_index),
        Form("Global overview of the %s variables",
    ...
    if (_make_profile_plots) {
      c_global_hist[ic] = new TCanvas(Form("global_profile_plots_%s_%d",
                                           ic == 0 ? "tracker" : (ic == 7 ? "pixel" : _sublevel_names[ic - 1].Data()),
                                           canvas_index),
                                      Form("Global overview profile plots of the %s variables",
                                           ic == 0 ? "tracker" : (ic == 7 ? "pixel" : _sublevel_names[ic - 1].Data())),
                                      _window_width,
                                      _window_height);
  ...
  cout << "Creation of the TGraph[" << x.size() << "][" << y.size() << "]["
...
#define INDEX_IN_GLOBAL_CANVAS(i1, i2) 1 + i1 + i2 *x.size()
  ...
  for (unsigned int ix = 0; ix < x.size(); ix++) {
    ...
    // looping on Y axes
    for (unsigned int iy = 0; iy < y.size(); iy++) {
      cout << "x[" << ix << "]=" << x[ix] << " and y[" << iy << "]=" << y[iy] << "\t-> creating TMultiGraph" << endl;
      mgraphs[ix][iy][0] = new TMultiGraph(
          Form("mgr_%s_vs_%s_tracker_%d",
               x[ix].Data(),
               y[iy].Data(),
               canvas_index),  // name
      ...
      for (unsigned short int jgraph = 0; jgraph < NB_SUBLEVELS * NB_Z_SLICES * n_module_types; jgraph++) {
        ...
        cout << "Cloning, coloring and adding TGraph " << _sublevel_names[igraph % NB_SUBLEVELS]
             << (igraph >= NB_SUBLEVELS ? "(z<0)" : "(z>0)") << " to global TMultiGraph" << endl;
      ...
      for (unsigned int isublevel = 1; isublevel <= NB_SUBLEVELS; isublevel++) {
        cout << "Cloning, coloring and adding TGraph " << _sublevel_names[isublevel - 1] << " to sublevel TMultiGraph"
             << endl;

        mgraphs[ix][iy][isublevel] =
            new TMultiGraph(Form("%s_vs_%s_%s_%d",
                                 x[ix].Data(),
                                 y[iy].Data(),
        ...
        if (graphs[ix][iy][isublevel - 1]->GetN() > 0)
          ...
          cout << "graphs[ix][iy][isublevel-1]=" << graphs[ix][iy][isublevel - 1]->GetName()
        ...
        if (graphs[ix][iy][NB_SUBLEVELS + isublevel - 1]->GetN() > 0)
          ...
          cout << "graphs[ix][iy][NB_SUBLEVEL+isublevel-1]=" << graphs[ix][iy][NB_Z_SLICES + isublevel - 1]->GetName()
               << " is empty -> not added into " << mgraphs[ix][iy][isublevel]->GetName() << endl;
      ...
      for (unsigned short int imgr = 0; imgr <= NB_SUBLEVELS + 1; imgr++) {
        ...
        c[ix][iy][imgr] =
            new TCanvas(Form("c_%s_vs_%s_%s_%d",
                             x[ix].Data(),
                             y[iy].Data(),
                             imgr == 0 ? "tracker" : (imgr == 7 ? "pixel" : _sublevel_names[imgr - 1].Data()),
                             canvas_index),
                        Form("%s vs. %s at %s level",
                             x[ix].Data(),
                             y[iy].Data(),
                             imgr == 0 ? "tracker" : (imgr == 7 ? "pixel" : _sublevel_names[imgr - 1].Data())),
                        _window_width,
  ...
  if (_make_profile_plots) {
    ...
    for (unsigned int ix = 0; ix < x.size(); ix++) {
      for (unsigned int iy = 0; iy < y.size(); iy++) {
          ...
          for (int binx = 0; binx <= histos2D[ix][iy][igraph]->GetXaxis()->GetNbins(); binx++) {
            ...
            histosYValues[ix][iy][igraph] =
                new TH1F("1Dhist_Y-Values" + x[ix] + y[iy] + _sublevel_names[igraph % NB_SUBLEVELS] +
                             TString(igraph % (NB_SUBLEVELS * NB_Z_SLICES) >= NB_SUBLEVELS ? "n" : "p") +
                             std::to_string(canvas_index) + std::to_string(binx),
                         "",
        ...
        c_hist[ix][iy][0] = new TCanvas(Form("c_hist_%s_vs_%s_tracker_%d", x[ix].Data(), y[iy].Data(), canvas_index),
                                        Form("Profile plot %s vs. %s at tracker level", x[ix].Data(), y[iy].Data()),
                                        _window_width,
                                        _window_height);
        ...
        if (_print && !_print_only_global)
          c_hist[ix][iy][0]->Print(
              _output_directory + Form("Profile_plot_%s_vs_%s_tracker_%d", x[ix].Data(), y[iy].Data(), canvas_index) +
        ...
        c_hist[ix][iy][7] = new TCanvas(Form("c_hist_%s_vs_%s_pixel_%d", x[ix].Data(), y[iy].Data(), canvas_index),
                                        Form("Profile plot %s vs. %s at pixel level", x[ix].Data(), y[iy].Data()),
                                        _window_width,
                                        _window_height);
        ...
        if (_print && !_print_only_global)
          c_hist[ix][iy][7]->Print(
              _output_directory + Form("Profile_plot_%s_vs_%s_pixel_%d", x[ix].Data(), y[iy].Data(), canvas_index) +
                  ExtensionFromPrintOption(_print_option),
              _print_option);
        ...
        for (unsigned int isublevel = 1; isublevel <= NB_SUBLEVELS; isublevel++) {
          ...
          c_hist[ix][iy][isublevel] =
              new TCanvas(Form("c_hist_%s_vs_%s_%s_%d",
                               x[ix].Data(),
                               y[iy].Data(),
                               isublevel == 0 ? "tracker" : _sublevel_names[isublevel - 1].Data(),
                               canvas_index),
                          Form("Profile plot %s vs. %s at %s level",
                               x[ix].Data(),
                               y[iy].Data(),
                               isublevel == 0 ? "tracker" : _sublevel_names[isublevel - 1].Data()),
                          _window_width,
          ...
          if (_print && !_print_only_global)
            c_hist[ix][iy][isublevel]->Print(_output_directory +
                                                 Form("Profile_plot_%s_vs_%s_%s_%d",
                                                      x[ix].Data(),
                                                      y[iy].Data(),
                                                      _sublevel_names[isublevel - 1].Data(),
                                                      canvas_index) +
